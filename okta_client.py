...
OKTA_ENVIRONMENTS = {
    "dev":  {"url": "https://host.example.gov",     "token_var": "OKTA_ADMIN_DEV_API_TOKEN"},
    "stg":  {"url": "https://host.example.gov", "token_var": "OKTA_ADMIN_STG_API_TOKEN"},
    "prod": {"url": "https://host.example.gov",         "token_var": "OKTA_ADMIN_PROD_API_TOKEN"},
}

# Only surface these app types — covers SAML/OIDC plus SWA (BROWSER_PLUGIN,
# AUTO_LOGIN, SECURE_PASSWORD_STORE, BOOKMARK) and WS-Fed. Some OIN replacements
# (e.g. ExampleApp) are SWA — without these, [REPLACED] rows look orphaned.
APP_SIGN_ON_MODES = {
    "SAML_2_0", "OPENID_CONNECT",
    "BROWSER_PLUGIN", "AUTO_LOGIN", "SECURE_PASSWORD_STORE", "BOOKMARK",
    "WS_FEDERATION",
}


class OktaClient:
    def __init__(self, base_url: str, api_token: str, session: requests.Session):
        self.base_url = base_url.rstrip("/")
        self._session = session
        self._session.headers["Authorization"] = f"SSWS {api_token}"
    ...
    def _get_sso_domain(self) -> str:
        """Return the custom SSO domain (e.g. host.example.gov). Cached per instance."""
        ...
        try:
            ...
            self._sso_domain = f"https://host.example.gov}" if custom else self.base_url
    ...
    _LABEL_SEARCH_MAP: dict[str, str] = {}  # internal mapping redacted for public source
    ...
    def search_oin_catalog(self, name: str, limit: int = 20) -> list[dict]:
        ...
        Filters applied:
          ...
          2. Word boundary — search term must appear as a whole word in the
             OIN display name (e.g. "ExampleApp" won't match "Gatekeeper")
        ...
        # and "UCC-ExampleApp" → key "ucc-ExampleApp" (hyphenated keys kept as-is).
        name_stripped = name.strip()
        raw_first = name_stripped.split()[0].lower() if name_stripped else ""
        # Try exact first-token match first (e.g. "ucc-ExampleApp"), then
        ...
        # Search and boundary-check on the first word — "ExampleApp Enterprise Plus"
